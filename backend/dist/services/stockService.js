...
function isTickerLikeName(name, symbol) {
...
function formatSymbolFallback(symbol) {
    const normalizedSymbol = symbol.toUpperCase();
    const baseSymbol = normalizedSymbol.split('.')[0];
    return KNOWN_COMPANY_NAMES[normalizedSymbol] || KNOWN_COMPANY_NAMES[baseSymbol] || baseSymbol;
}
async function resolveCompanyName(alphaVantageApiKey, symbol, overviewData) {
    const normalizedSymbol = symbol.toUpperCase();
    ...
    try {
        const searchResponse = await axios_1.default.get(BASE_URL, {
            params: {
...
async function getStockData(symbol) {
    try {
        ...
        const quoteResponse = await axios_1.default.get(BASE_URL, {
            params: {
        ...
        const overviewResponse = await axios_1.default.get(BASE_URL, {
            params: {
                function: 'OVERVIEW',
                symbol: symbol,
                apikey: alphaVantageApiKey
